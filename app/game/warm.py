# https://www.youtube.com/watch?v=rtwtOcfYKqc&ab_channel=이수안컴퓨터연구소
...
def show_info(length, speed, surface):
    ...
    text = font.render("Length: " + str(length) +
                       "   Speed: " + str(round(speed, 2)), 1, GRAY)
    pos = text.get_rect()
    pos.centerx = 150
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption("Warm Game")
    ...
    while True:
        ...
        warm.move()
        check_eat(warm, feed)
        speed = (FPS + warm.length) / 2
        show_info(warm.length, speed, surface)
        warm.draw(surface)
        feed.draw(surface)
        window.blit(surface, (0, 0))
        pygame.display.flip()
